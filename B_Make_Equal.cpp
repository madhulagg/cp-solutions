...
string is_possible(int n, vector<int>& containers) {
    long long total_water = 0;
    for (int i = 0; i < n; ++i) {
        total_water += containers[i];
    }
    long long target_water = total_water / n;
    if (total_water % n != 0) {
        return "NO";
    }
    long long excess_water = 0;
    for (int i = 0; i < n; ++i) {
...
signed main(){
    ...
    while(testcase--){
        ...
        cout << is_possible(n, containers) << endl;
    }
}
